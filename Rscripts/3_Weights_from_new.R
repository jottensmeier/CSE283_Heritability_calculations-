...
prepare_gene_table <- function(gtf, gene_list) {
  ...
  setnames(df, old = c("gene_id", "gene_name", "gene_type"),
...
extract_LD_info_from <- function(matrix_path, var_path, snps.keep) {
  ...
  if (!length(keep)) {
    return(data.table(RSID1=character(), RSID2=character(), LD=numeric()))
  }
...
qtls_by_chromosome <- function(qtl_table, LD_path_map_by_chr) {
  ...
  qtl_table_by_chr <- qtl_table[
    , .(rsids = list(unique(rsid))), by = Chromosome ]

  snps_by_chr <- setNames(
    qtl_table_by_chr$rsids,
...
main <- function(qtl_path, qtl_data_base, gtfpath, LD_directory, outdir) {
  ...
  data.table::setnames(cov_table, old = c("gene", "LD"), new = c("GENE", "VALUE"))
